...
// POST to approve a concept image direction
export async function POST(
  ...
  let body: { action?: string; image_id?: string; approved_image_path?: string }
  ...
  if (body.action === 'approve_vendor_checkout') {
    ...
    // Enqueue checkout gate job with approval
    const { nanoid } = await import('nanoid')
    const jobId = nanoid(21)
