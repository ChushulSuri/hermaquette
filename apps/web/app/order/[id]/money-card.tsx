    ...
    <div className="bg-gray-900 border border-amber-800/50 rounded-xl p-5">
      ...
      <div className="space-y-2 text-sm">
        <div className="flex justify-between text-gray-300">
          ...
          <span className="font-mono">{cents(ledger.vendor_cost_cents)}</span>
        </div>
        <div className="flex justify-between text-gray-300">
          <span>Service fee (10%)</span>
          <span className="font-mono text-amber-300">{cents(ledger.service_fee_cents)}</span>
        </div>
        <div className="border-t border-gray-700 pt-2 flex justify-between font-semibold text-white">
          <span>Customer price</span>
          <span className="font-mono text-xl">{cents(ledger.revenue_cents)}</span>
        </div>
        <div className="flex justify-between text-xs text-gray-500 pt-1">
          <span>Gross margin pre-fees</span>
          <span className="font-mono text-green-400">{cents(ledger.gross_margin_pre_fees_cents)}</span>
        </div>
