 ...
 * Exit: 0 on success, 1 on printability failure or fatal error
 */
 ...
 */
async function resolveStlPath(stlPath) {
  if (!stlPath) throw new Error(`No STL path available for order ${orderId}`)

  // file:// → strip prefix, use local path directly
  if (stlPath.startsWith('file://')) {
    return stlPath.slice('file://'.length)
  }

  // HTTP(S) URL → download to temp file
  if (stlPath.startsWith('http://') || stlPath.startsWith('https://')) {
    const tmpDir = join(tmpdir(), 'hermaquette-quote')
    try { mkdirSync(tmpDir, { recursive: true }) } catch {}
    const tmpStl = join(tmpDir, `${orderId}_${Date.now()}.stl`)
    const resp = await fetch(stlPath, { signal: AbortSignal.timeout(60_000) })
    if (!resp.ok) throw new Error(`Cannot download STL: HTTP ${resp.status} ${stlPath}`)
...
if (quoteResult.quote_source !== 'manual') {
  ...
  if (printability == null) {
    ...
    console.error(JSON.stringify({ error: 'Vendor printability verdict absent — cannot auto-proceed to checkout' }))
    process.exit(1)
  }
  ...
  if (!isPrintable) {
    ...
    console.error(JSON.stringify({ error: `Vendor printability check failed: ${printability}` }))
    process.exit(1)
  }
}
...
// Idempotent: check for existing ledger row before inserting
...
console.warn(`[vendor-quote] order=${orderId} ledger=${ledgerId} revenue=${revenueCents}¢ source=${quoteResult.quote_source}`)
